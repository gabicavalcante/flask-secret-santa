...
def process_message(message, number):
    ...
    :return: list of dict in the form [{ "to": "...", "message": "..."}]
    """
    response = []
    ...
    if message == "create draw":
        ...
        return _bot_replay("message")

    if "want to join the draw" in message:
        participant_name = message.split("want to join the draw")[0]
        code = message.split("want to join the draw")[-1]
